...
## Usage : perl $0 name_CpG[CHH|CHG].methylKit TileSize Depth Type
## Xue Xiaohui 
...
if (@ARGV != 5 ){
	print "Usage : perl $0 <methylKit_methLevel> <bin_length> <depth> <type> <outfile>\n";
	exit;
}

open IN,$ARGV[0] or die "Please input methylKit file";## This is used for methylation site info in methylKit format
my $window = $ARGV[1]; ## bp
...
while(<IN>){
  ...
  next if $_=~/Lambda/;
	my @lines = split(/\s+/,$_);
	my($chr,$pos,$total,$meth,$ratio)= ($lines[1],$lines[2],$lines[4],int($lines[4]*$lines[5]/100), $lines[5]/100);
	next if ($total < $Depth);
